...
public class CmdPing extends DornacraftCommand {
	...
	private static final String CMD_DESC = "Vous indique la latence du joueur ciblé.";
			...
			public void execute(CommandSender sender, Command cmd, String label, String[] args) throws Exception {
				...
				if (args.length == 0
					...
					if (sender instanceof Player) {
						UtilsAPI.sendSystemMessage(MessageLevel.INFO, sender, "§6Pong ! §8%s §c%s ms",
								UtilsAPI.CARAC_DOUBLE_QUOTE_END, getPing((Player) sender));
					} else {
					...
					if (target != null) {
						UtilsAPI.sendSystemMessage(MessageLevel.INFO, sender,
								"Temps de réponse du joueur §b%s §e: §c%s ms", target.getDisplayName(),
								getPing(target));
					} else {
